#!/usr/bin/env python3
"""Build focus-airport dataset from OurAirports.

Small GA fields used as focus points aren't "large_airport" so they're
excluded from the general large-airports overlay. This script bakes a
supplementary table containing only these ICAOs, plus their runways.
The runway_overlay renders these ONLY when the current focus matches,
so they add zero clutter to the general Home view.
"""
...
OUT_H = ROOT / "include" / "data" / "focus_airports.h"
OUT_CPP = ROOT / "src" / "data" / "focus_airports_data.cpp"
...
# Hand-picked ICAOs. Every entry here must correspond to a name in
# services::focus::kFocusPoints so the runway overlay can match by ident.
FOCUS_IDENTS: frozenset[str] = frozenset({
    "KSQL",  # San Carlos
    "KHAF",  # Half Moon Bay
    "KPAO",  # Palo Alto
    "KHWD",  # Hayward
