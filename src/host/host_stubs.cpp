...
// --- Emulator bootstrap tile ----------------------------------------------
// On real hardware the device fetches its map tile over HTTPS (milestone
// 2 step 11); on the SDL emulator we short-circuit that by loading a
// pre-baked SF-area tile from disk at boot. Path is relative to the
// PlatformIO working directory (repo root), matching how
// data/ui_font.vlw is loaded via .incbin above.
//
// The tile is regenerated by:
//   python scripts/build_tiles.py --zoom 7 --out-dir /tmp/tile-smoke
//   cp /tmp/tile-smoke/tiles/7/20/37.bin data/emulator_bootstrap_tile_7_20_37.bin
namespace host {

void loadBootstrapTiles() {
  const char* path = "data/emulator_bootstrap_tile_7_20_37.bin";
  FILE* f = std::fopen(path, "rb");
  if (f == nullptr) {
    std::printf("host: no bootstrap tile at %s (falling back to flash overview)\n",
                path);
    return;
  }
